...
use Bitrix\Main\Diag\Debug as dbg;
...
if ((int)$ORDER_ID <= 0)
{
	...
	*/
	...
	$arFilter = array("ACTIVE" => "Y", "UTIL" => "N");
	...
	while ($arProperties = $dbProperties->GetNext())
	{
		...
		// if ($arProperties["REQUIED_FORMATED"] == "Y" AND $curVal == "")
			//$errorMessage .= str_replace("#NAME#", $arProperties["ID"] == 1 ? "���� ���" : $arProperties["NAME"], GetMessage("SOE_EMPTY_PROP"))."<br>";
	...
	*/
	...
	{
	//                $payerEMail = "user_".time().GetRandomCode(2)."@a-cab.ru";
	...
	//			$errorMessage .= GetMessage("STOF_ERROR_REG_UNIQUE_EMAIL")."<br>";
		...
		if ('' == $errorMessage)
		{
			$user_id = CSaleUser::DoAutoRegisterUser("user_".time().GetRandomCode(2)."@a-cab.ru", $payerName, SITE_ID, $arErrors, array());
	...
	*/
	...
	if ($_REQUEST["BasketRefresh"] <> '' || $_REQUEST["BasketOrder"] <> '' || $_REQUEST["AJAX_CALL"] <> '')
	{
		if (in_array("QUANTITY", $arParams["COLUMNS_LIST"]))
		{
			...
			{
				//dbg::dumpToFile($val, 'arFields', $fileName = '/local/logs/~logs.log');
		...
		if ($PERSON_TYPE == 1) {
			...
			foreach($arShoppingCart as $key => $val)
			{
				...
				if ($ar_res = $db_res->Fetch())
				{
					...
					$updateResult = CSaleBasket::Update($val["ID"], $arFields);
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["BasketOrder"]) AND $errorMessage == '' AND check_bitrix_sessid())
	{
		...
		if ('' == $errorMessage)
		{
			...
			// dbg::writeToFile($arBasketItems, 'arFields', $fileName = '/local/logs/~logs.log');
			dbg::writeToFile($arBasketItems, 'arFields', $fileName = '/local/logs/~logs.log');
			$ORDER_ID = CSaleOrder::DoSaveOrder($arBasketItems, $arAdditionalFields, 0, $arErrors);


			// dbg::writeToFile($_POST, 'arFields', $fileName = '/local/logs/~logs.log');
			// dbg::writeToFile($arBasketItems, 'arFields', $fileName = '/local/logs/~logs.log');
			...
			if ($ORDER_ID > 0 && empty($arErrors))
			{
				...
				$arFields = array(
					...
					"COMMENT_USER" => htmlspecialchars($_REQUEST["ORDER_DESCRIPTION"]),
					"PHONE_USER" => $arPropValues[3],
					"NAME_USER" => $arPropValues[1],
				);
				/*
				...
				*/
				...
				//dbg::dumpToFile($arFields, 'arFields', $fileName = '/local/logs/~logs.log');
	...
	if (is_array($arBasketItems["BASKET_ITEMS"]))
	{
		foreach ($arBasketItems["BASKET_ITEMS"] as $key => $val)
		{
			...
			if($val["DISCOUNT_PRICE"]>0 || $val["PRICE"]>0) $val["DISCOUNT_PRICE_PERCENT"] = $val["DISCOUNT_PRICE"]*100 / ($val["DISCOUNT_PRICE"] + $val["PRICE"]);
	...
	if (is_array($arBasketItems["BASKET_ITEMS"]))
	{
		...
		*/
		...
		foreach ($arOrderForDiscount['BASKET_ITEMS'] as &$arOneItem)
		{
			...
			if($arOneItem["DISCOUNT_PRICE"] > 0 || $arOneItem["PRICE"] > 0) $arOneItem["DISCOUNT_PRICE_PERCENT"] = $arOneItem["DISCOUNT_PRICE"]*100 / ($arOneItem["DISCOUNT_PRICE"] + $arOneItem["PRICE"]);
